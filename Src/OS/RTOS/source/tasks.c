...
/*-----------------------------------------------------------*/
	...
	unsigned short usCurStack = 0;
		...
		do
		{
			...
			#endif			
			if(pxNextTCB->pcTaskName == NULL) 
			{
				sprintf( pcStatusString, ( char * ) "Prior%u\t\t%c\t\t%u\t\t%u\t\t%u\t\r\n", ( unsigned int ) (configMAX_PRIORITIES - pxNextTCB->uxPriority),  cStatus, ( unsigned int )pxNextTCB->stacksize, ( unsigned int )usCurStack,( unsigned int )usStackRemaining );
			}
			else
				sprintf( pcStatusString, ( char * ) "%s\t\t%c\t\t%u\t\t%u\t\t%u\t\r\n", ( unsigned int ) (pxNextTCB->pcTaskName),  cStatus, ( unsigned int )pxNextTCB->stacksize, ( unsigned int )usCurStack,( unsigned int )usStackRemaining );
