...
#define TLS_ONESHOT_RECV_TIME   30000*HZ/1000//50000*HZ/1000
#define TLS_ONESHOT_SWITCH_TIMER_MAX (100*HZ/1000)//(80*HZ/1000)
...
int tls_wifi_lsd_oneshot_special(u8 *data, u16 data_len)
{
	...
	{
		printf(" tls_wifi_lsd_oneshot_special is_airkiss:%d is_oneshot:%d\n",is_airkiss,is_oneshot);
		if(is_airkiss != TRUE)
		{
			is_oneshot = TRUE;
...
void tls_oneshot_task_handle(void *arg)
{
    ...
    for(;;)
    {
        ...
        switch((u32)msg)
        {
           ...
           if (0 == gucConfigMode) 
		   {
#if TLS_CONFIG_AIRKISS_MODE_ONESHOT || TLS_CONFIG_UDP_LSD_SPECIAL  
				if ((is_airkiss == TRUE) || ( is_oneshot ==TRUE))
				{
