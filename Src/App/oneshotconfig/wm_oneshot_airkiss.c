...
static airkiss_result_t result = {0};
...
extern void client_set_setup_token(const char *);

char airkiss_device_id[CONF_DEV_ID_MAX];
char airkiss_device_type[CLIENT_SETUP_TOK_LEN];
...
void adw_wifi_start_aks(void *wifi)
{
  ...
  //tls_airkiss_start();
	tls_wifi_set_oneshot_config_mode(0);//udp
...
static void data_xor_encode(char * data, char key, int len)
{
 int i;
 for(i = 0; i < len; i++)
 {
...
void oneshot_airkiss_send_reply(void)
{
    ...
    struct sockaddr_in addr;
		u8 dsn_len = strlen(airkiss_device_id);
		
    if (is_airkiss || is_oneshot)
    {
    	is_oneshot = FALSE;
			is_airkiss = FALSE;
			//printf("airkiss:%d oneshot:%d\n",is_airkiss,is_oneshot);
			//printf("encry ago:%s %d %s %d\n",airkiss_device_id,strlen(airkiss_device_id),airkiss_device_type,strlen(airkiss_device_type));
			data_xor_encode(airkiss_device_id,random4reply,strlen(airkiss_device_id));
			data_xor_encode(airkiss_device_type,random4reply,strlen(airkiss_device_type));
    }
    else
    {
        return ;
    ...
    {
        char nbuf[CONF_DEV_ID_MAX + CLIENT_SETUP_TOK_LEN + 1] = { 0 };
        u16 i=0;
        u16 len = 0;
        ...
        if( strlen(airkiss_device_id)<sizeof(airkiss_device_id) )
        {
						{
							nbuf[i++] = dsn_len;
							memcpy(&nbuf[i],airkiss_device_id,dsn_len);
							i += dsn_len;
							if( strlen(airkiss_device_type)<sizeof(airkiss_device_type) )
							{
									nbuf[i++] = 8;
									memcpy(&nbuf[i],airkiss_device_type, 8);
									i += 8;
							}
						}
    ...
    //    printf("\ntime_start:%d",clock_get(NULL));
        for ( i = 0; i <= 60; i++)
        {
            ...
            sendto(socket_num, nbuf, len, 0, (struct sockaddr*) &addr, sizeof(struct sockaddr_in));
            //tls_os_time_delay(40);
            //printf("\n nothing do do...\n");
            tls_os_time_delay(30);
        }
//        printf("\ntime_stop:%d",clock_get(NULL));
//      printf("\nsend dsn+token is over...\n\n");		
    }
#else
...
void oneshot_airkiss_finish(void)
{
		if (is_airkiss == TRUE)
		{
  	  int ret = -1;
	    ...
	    printf(" OK smartconfig--->start connect: ssid '%s', ssid_len '%hhu', pwd '%s', pwd_len '%hhu', random '%hhu'.\r\n", result.ssid,
		...
		{
		 	is_airkiss = FALSE;
			printf(" OK simpleconfig-->start connect: ssid '%s', ssid_len '%hhu', pwd '%s', pwd_len '%hhu', random '%hhu'.\r\n", lsd_param.ssid,
			lsd_param.ssid_len,lsd_param.pwd,lsd_param.pwd_len,lsd_param.user_data[0]);
			random4reply = lsd_param.user_data[0];
		}
#ifdef AIRKISS
    {
        unsigned char i,temp;
        unsigned char r[9];
        extern char conf_sys_dev_id[CONF_DEV_ID_MAX];
        
        //读取 DSN 到 airkiss_device_id
        ...
        //生成 setup_token 到 airkiss_device_type
        memset(airkiss_device_type,0,sizeof(airkiss_device_type));
        u8 *mac =tls_mem_alloc(6);
        mac = wpa_supplicant_get_mac();
        srand(tls_os_get_time());

    //  if(is_oneshot == TRUE ||is_airkiss == TRUE )
      {
        for(i=0;i<7;i++)
        {
					r[i] = (u8)rand() + mac[i] + random4reply;
				}
					r[7] = tls_os_get_time() % 255 ;
					r[8] = '\0' ;
				tls_mem_free(mac);
        for(i=0;i<8;i++)
        {
        #if 1
            //0-9 a-z 36 A-Z 36+26 = 62
            temp = r[i]%61; /*0-35*/
            if(temp<10)
            {
            ...
            {
							airkiss_device_type[i] = 'A' + temp - 36;
            }
            #endif
        ...
        client_set_setup_token(airkiss_device_type);//载入到 ada 中
    }
    tls_netif_add_status_event(wm_oneshot_netif_status_event);
		...
		if(!(is_airkiss) && (is_oneshot))
			adw_wifi_aks_setup(lsd_param.ssid,lsd_param.ssid_len,lsd_param.pwd,lsd_param.pwd_len);
    return ;
}
#else
//-------------------------------------------------------------------------
...
void tls_airkiss_recv(u8 *data, u16 data_len)
{
    ...
    else if ((ret == AIRKISS_STATUS_COMPLETE)&&(is_airkiss == FALSE))/* 9.已经接收到了所有的配置数据 */
    {
        ...
        printf(" tls_airkiss_recv is_airkiss:%d is_oneshot:%d\n",is_airkiss,is_oneshot);
        if(is_oneshot != TRUE)
        {
        	is_airkiss = TRUE;
