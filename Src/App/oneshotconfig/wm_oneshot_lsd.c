...
#define LSD_DATA_OFFSET			18//18//20
...
const u8 lsd_dst_addr[5] = {0x01,0x00,0x5e,0x63,0x59};
...
static void data_xor_decode(char * data, char random, int len)
{
 int i;
...
int tls_lsd_recv(u8 *buf, u16 data_len)
{
	...
	//if(memcmp(multicast, lsd_dst_addr, 5) &&  hdr->duration_id)
	if(memcmp(multicast, lsd_dst_addr, 5))
	{
		return LSD_ONESHOT_CONTINUE;
	}
	...
	switch(lsd_state)
	{
		case 0:
			...
			if(is_zero_ether_addr(lsd_src_mac))
			{
				...
				if(lsd_printf)
					lsd_printf("tods:%d,%d,"MACSTR","MACSTR"\n", tods, frm_len, MAC2STR(SrcMac),MAC2STR(multicast));
			}
			else
			{
			...
			if(lsd_sync_cnt >= 1)
			{
				...
				if(lsd_printf)
					lsd_printf("guide:%d %d %d %d\n",lsd_head[tods][0],lsd_head[tods][1],lsd_head[tods][2],lsd_head[tods][3]);
		...
		case 1:
			...
			{	
				//printf("recv app:%d %d %d %d\n",lsd_byte[tods][0],lsd_byte[tods][1],lsd_byte[tods][2],lsd_byte[tods][3]);
			...
			if(lsd_data.used[0] && lsd_data.used[1] && lsd_data.used[2])
			{
				...
				//printf("automan:totalLen:%d pwdLen:%d ssidLen:%d\n",totalLen,pwdLen,ssidLen);
				...
				if(lsd_data_cnt >= totalLen + 2)
				{
					...
					if((ssidLen==0) && (pwdLen==0))				//only userData
					{
						...
						data_xor_decode(lsd_param.ssid,lsd_param.user_data[0],lsd_param.ssid_len);
						data_xor_decode(lsd_param.pwd,lsd_param.user_data[0],lsd_param.pwd_len);	
					...
					if(pwdLen > 0)
					{
						ssidCrc = lsd_data.data[4];
						memcpy(lsd_param.ssid,&lsd_data.data[5],ssidLen);
						memcpy(lsd_param.pwd,&lsd_data.data[6+ssidLen],pwdLen);
					...
					//ayla_ssid_pwd_decrypt(lsd_param.ssid,lsd_param.ssid_len,lsd_param.pwd,lsd_param.pwd_len,lsd_param.user_data[0]);
					//printf("++:%s %s %d\n",lsd_param.ssid,lsd_param.pwd,lsd_param.user_data[0]);
					data_xor_decode(lsd_param.ssid,lsd_param.user_data[0],lsd_param.ssid_len);
					data_xor_decode(lsd_param.pwd,lsd_param.user_data[0],lsd_param.pwd_len);
					return LSD_ONESHOT_COMPLETE;
				}	//have no userData
				else if(ssidLen > 0)
				{
					...
					if(lsd_data.used[3])			//bssidCrc
					{
						...
						if(pwdLen > 0)
						{
							if(0 == lsd_data.used[5+ssidLen])
							{
								return LSD_ONESHOT_CONTINUE;
							}
							pwdCrc = lsd_data.data[5+ssidLen];
							for(i=0; i<pwdLen; i++)
							{
								if(lsd_data.used[6+ssidLen+i])
								{
									lsd_param.pwd[i] = lsd_data.data[6+ssidLen+i];
								}
								else
								{
									break;
							...
							if(pwdCrc != lsd_crc8_calc(&lsd_data.data[6+ssidLen], pwdLen))
							{
								if(lsd_printf)
	...
	switch(lsd_state)
	{
		...
		case 1:
			...
			if(lsd_data.used[0] && lsd_data.used[1] && lsd_data.used[2])
			{
				...
				pwdLen = lsd_data.data[1];
				ssidLen = lsd_data.data[2];
				//printf("totalLen:%d pwdLen:%d ssidLen:%d\n",totalLen,pwdLen,ssidLen);
				if((ssidLen > 32) || (pwdLen > 64))
				{
					return LSD_ONESHOT_ERR;
				}
