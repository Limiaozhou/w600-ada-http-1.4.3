...
void * mem_alloc_debug(u32 size, char* file, int line)
{
	...
	if(buffer == NULL)
	{
		printf("malloc %d:%s,%d; %d\n", size, file, line, cpu_count);
	}
	return buffer;
...
void mem_free_debug(void *p)
{
	...
	if ((p < 0x20000000)||(p > &__heap_limit))
	{
	    printf("free strange: %x, %x", p,  &__heap_limit);
		return;
	}
