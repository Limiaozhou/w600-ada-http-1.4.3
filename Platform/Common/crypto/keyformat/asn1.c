					 ...
					 psRsaKey_t *pubKey)
{
    ...
    */
	if (len < 1) {
		psTraceCrypto("Initial parse error in 1\n");
		return PS_PARSE_FAIL;
	}
    if ((*(p++) != ASN_BIT_STRING)) {
		psTraceCrypto("Initial parse error in 2\n");
		return PS_PARSE_FAIL;
	}
    if (getAsnLength(&p, len - 1, &pubKeyLen) < 0) {
		psTraceCrypto("Initial parse error in 3\n");
		return PS_PARSE_FAIL;
	}
    if ((len - 1) < pubKeyLen) {
		psTraceCrypto("Initial parse error in 4\n");
		return PS_PARSE_FAIL;
	}
