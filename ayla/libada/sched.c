 ...
 */
static int sched_actions_run(struct sched_state *st)
{
...
{
#ifndef SCHED_TEST
	return get_ua_with_len((u8 *)atlv + 2, atlv->len, (u32 *)value);
#else
	switch (atlv->len) {
	case 1:
		*value = *(u8 *)(atlv + 1);
...
#if 0
	if(sched_actions_run(st))
	{
		adap_sched_run_time_write(sched_state.run_time + 1);
	}
#endif	
}
