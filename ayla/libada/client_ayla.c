...
const char ada_version_build[] = //"ADA " ADA_VERSION BUILD_NAME " " BUILD_SDK
			...
			SUNSEA_PID " "
#endif			
			"ADA"" "ADA_VERSION BUILD_NAME " " BUILD_SDK
#ifdef PLATFORM_BUILD_SDK
			" " PLATFORM_BUILD_SDK
#endif
#ifdef SDK_VERSION
			"-" SDK_VERSION
...
char new_filter_rule = 0,filter_times = 0;
...
static void client_sync_time_wait(void);
static int client_batch_status_accept(struct ame_decoder *dec,void *arg, struct ame_kvp *kvp);
static int client_batch_id_accept(struct ame_decoder *dec,
...
static int client_reg_type(struct xml_state *sp, int argc, char **argv)
{
	...
	client_parse_str(state->reg_type, sizeof(state->reg_type), argc, argv);
	return 0;
}
...
static int client_reg_token(struct xml_state *sp, int argc, char **argv)
{
	...
	client_parse_str(cf->reg_token, sizeof(cf->reg_token), argc, argv);
	return 0;
}
...
static void client_prop_cmd_send(struct client_state *state)
{
	...
	state->prop_send_cb(PROP_CB_BEGIN, 0 , NULL);
}
...
{
	if ((!state->serv_conn || state->ping_ssl)
	    && (CLIENT_HAS_KEY(state) || state->ping_time)) {
...
static void client_sync_time_wait(void)
{
	...
	random_fill(&ran_tm, sizeof(ran_tm));
	/* random sync time after 22 hours */
	delay = ((22 * 3600) + (ran_tm >> 2)) << 10;

	client_timer_set(&state->sync_time_timer, delay);
}

static void client_sync_time_timeout(struct timer *timer)
{
	struct client_state *state = &client_state;
...
void client_sync_time_enable(int enabled)
{
	...
	state->sync_time = (enabled != 0);
	if (!state->sync_time) {
		client_timer_cancel(&state->sync_time_timer);
	}
}
...
void client_wakeup(void)
{
	...
	ASSERT(client_locked);
    //printf("client_wakeup: %x\r\n", state->next_step_cb.func);
...
void client_tcp_recv_done(struct client_state *state)
{
	...
	if (hc->http_time) {
		now = clock_ms();
		if (clock_gt(now, state->time_sync_time)) {
			state->time_sync_time = now + CLIENT_CONF_MIN_SYNC;
...
enum ada_err client_recv_cmds(struct http_client *hc, void *payload, size_t len)
{
	...
	filter_times ++; //needn't chean zero
	//printf("filter_times:%d\n\n\n",filter_times);
	if(filter_times >= 3)
	{
		if(new_filter_rule != 0)
			new_filter_rule = 0; //clear new_filter_rule
	}
	client_log(LOG_DEBUG "client_recv_cmds GET done");

	if(new_filter_rule == 1)  //continue get cmd 
	{	
		client_timer_set(&state->cmd_timer, 0);
		state->prefer_get = 1;
	}
	else
	{	
...
static void client_get_cmds(struct client_state *state)
{
	...
	if (state->get_all) {
		new_filter_rule = 1;//set filter rule 's value
		filter_times = 0;
...
/*
 * Given a location in format /<prop_id>/<dp_id>, returns a string
 * in the format properties/<prop_id>/datapoints/<dp_id>
 */
	...
	char *dest, int dest_len)
{
	...
	prop_id[len] = '\0';
	snprintf(dest, dest_len, "/properties%s/datapoints%s", prop_id, dp_id);
...
/*
 * Handle response for fetched datapoint post.
 */
					...
					void *payload, size_t len)
{
	...
	if (!payload) {
		msg = "OEM auth";
		if (state->client_info_flags & CLIENT_UPDATE_OEM) {
			if (hc->hc_error != HC_ERR_NONE) {
				client_log(LOG_WARN "%s failed, err %u.", msg,
				    hc->hc_error);
			} else {
				client_log(LOG_WARN "%s passed.", msg);
			}
		}
...
#ifdef AYLA_BATCH_PROP_SUPPORT
static u8 client_batch_send_is_over(void)
{
	struct batch *b;
	struct prop *prop = client_state.batch_prop;
	...
	return batch_is_all_dps_sent(b);
}
#endif
/*
	...
	if (err == AE_OK) {
		if(!hc->chunked_eof)
		{
			...
			if(hc->http_tx_chunked)
			{
				...
				{
					client_prop_send_continue(state, NULL);
				}
				return;
			}
#endif
...
void client_send_post(struct http_client *hc)
{
	...
	err = state->prop_send_cb(PROP_CB_BEGIN, 0,  NULL);
...
void client_send_callback_set(enum ada_err (*callback)(enum prop_cb_status stat,
				u16,void *arg), struct prop *prop)
{
	struct client_state *state = &client_state;
#ifdef AYLA_LAN_SUPPORT
	struct ada_lan_conf *lcf = &ada_lan_conf;
	u8 lan_enable = lcf->enable;
#else
	u8 lan_enable = 0;
#endif
	struct http_client *hc = &state->http_client;
	...
	if (!(prop->send_dest) && !lan_enable) {
		/* Try sending to ADS even if we're not connected */
		state->dest_mask |= NODES_ADS;
	} else if (prop->send_dest) {
		state->dest_mask &= (prop->send_dest);
		state->failed_dest_mask = (prop->send_dest) &
...
static void client_err_cb(struct http_client *hc)
{
	...
	case HC_ERR_HTTP_STATUS:
		...
		if (hc->http_state.status == HTTP_STATUS_NOT_FOUND) 
		{
...
void client_init(void)
{
	...
	timer_init(&state->sync_time_timer, client_sync_time_timeout);
	state->sync_time = 1;
#ifdef AYLA_LAN_SUPPORT
...
enum wifi_error client_status(void)
{
    ...
    CLIENT_DEBUG(LOG_INFO, "client_status: %d", hc->hc_error);
	switch (hc->hc_error) {
	case HC_ERR_NONE:
...
struct http_client* http_content(void)
{
	struct client_state *state = &client_state;
	struct http_client *hc = &state->http_client;
...
#if   1
void client_keep_tls_by_jv_ctrl(void)
{
#ifdef JV_CTRL
	prop_send_by_name("jv_ctrl");
#endif	
}

void ext_get_cmd(void)
{
	//struct client_state *state = &client_state;
	//client_get_cmds(state);
	//client_get_ping(state);
	client_keep_tls_by_jv_ctrl();
}
#endif
#ifdef AYLA_BATCH_PROP_SUPPORT
static const char batch_tag_dsn[] = "dsn";
...
static const struct ame_tag client_batch_prop_tag_list[] = {
	AME_TAG(batch_tag_dsn, NULL, NULL),
	AME_TAG(batch_tag_name, NULL, NULL),
	AME_TAG(batch_tag_type, NULL, NULL),
	AME_TAG(batch_tag_value, NULL, NULL),
	AME_TAG(batch_tag_time, NULL, NULL),
...
static const struct ame_tag client_batch_dps_tag_list[] = {
	...
	AME_TAG(batch_tag_property, client_batch_prop_tag_list, NULL),
	AME_TAG(NULL, NULL, NULL)
};
...
static int client_batch_status_accept(struct ame_decoder *dec,
		void *arg, struct ame_kvp *kvp)
{
	struct batch_dps_rsp *bdpr = (struct batch_dps_rsp *) arg;
...
{
	struct ame_encoder *enc = &client_state.ame.enc;
...
static u8 client_ame_parsed_buf[1024 + 512];
static struct ame_kvp client_ame_stack[30];

enum ada_err client_send_batch_dp(struct prop *prop)
{
	int cnt;
	char uri[CLIENT_GET_REQ_LEN];
	struct client_state *state = &client_state;
	...
	if (!hc->prop_callback) {
		...
		hc->http_tx_chunked = 1;/*modify by 2019.11.5*/
		hc->chunked_eof = 0;

		hc->body_buf = state->buf;
		hc->body_len = 0;     /* len in of body */
		hc->body_buf_len = 0;
	...
	if (b->dps_tx >= 0 && b->dps_tx < b->dps_cnt) {
		...
		ASSERT(cnt <= size);
		err = http_client_send(hc, buf, cnt);
		if (!err && last) {
			err = http_client_send(hc, "", 0);
		}
		if (buf != state->buf) {
			tls_mem_free(buf);
		}
...
on_error:
	...
	b->dps_tx += 1;
	if (b->dps_tx >= b->dps_cnt) {
		hc->http_tx_chunked = 0;
		hc->chunked_eof = 1;
		hc->prop_callback = 0;
		client_log(LOG_DEBUG2 "Last Chunk Sent.\n");
	}
	return err;
}
