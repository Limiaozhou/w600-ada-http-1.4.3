...
/* actually public key in base-64 */
static u8 ada_pub_key_buf[400];		
...
void ada_conf_id_cli(int argc, char **argv)
{
	...
	if (rc == CONF_ERR_PERM) {
		printcli("id: conf not permitted");printcli("para set err\r\n");
		return;
	}
	if (rc == CONF_ERR_RANGE) {
		printcli("id: invalid value");printcli("para set err\r\n");
		return;
	}
	if (rc != CONF_ERR_NONE) {
		printcli("id: conf err %d", rc);printcli("para set err\r\n");
		return;
	}
	conf_id_set = 1;
