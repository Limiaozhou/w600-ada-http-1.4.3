...
static void client_ame_traverse(struct ame_state *ctx,
	const struct ame_tag *tag, struct ame_kvp *kvp)
{
	const struct ame_tag *sub = NULL;
	const struct ame_tag *tmp;

	switch (kvp->type) {
	case AME_TYPE_OBJECT:
	case AME_TYPE_ARRAY:
		if (kvp->key) {
			tmp = client_ame_find_tag(tag, kvp);
			if (tmp) {
				sub = tmp->subtags;
			}
		}
		break;
	case AME_TYPE_BOOLEAN:
	case AME_TYPE_UTF8:
	case AME_TYPE_INTEGER:
	case AME_TYPE_DECIMAL:
	case AME_TYPE_FLOAT:
	case AME_TYPE_NULL:
		tmp = client_ame_find_tag(tag, kvp);
		if (tmp) {
			if (tmp->cb) {
				tmp->cb(&ctx->dec, ctx->cb_arg, kvp);
			}
		}
		break;
	case AME_TYPE_UNKNOWN:
	case AME_TYPE_COUNT:
	case AME_TYPE_OPAQUE:
	default:
		break;
	}

	if (kvp->next) {
		client_ame_traverse(ctx, tag, kvp->next);
	}
	...
	if (kvp->child) {
		...
		client_ame_traverse(ctx, sub, kvp->child);
	}
}
...
enum ada_err client_ame_init(struct ame_state *ctx,
		...
		const struct ame_tag *list, void *cb_arg)
 {
	...
	err = ame_json_enc_init(&ctx->enc);
	if (err != AE_OK) {
		return err;
	}
	err = ame_json_dec_init(&ctx->dec);
	if (err != AE_OK) {
		return err;
	}
...
{
	for (; tag && tag->name; tag++) {
		if (!memcmp(kvp->key, tag->name, kvp->key_length)) {
			return tag;
		}
	}
	return NULL;
		...
		const u8 *data, int len)
{
	...
	struct ame_decoder *dec = &ctx->dec;
	struct ame_kvp *kvp;
	...
	while (1) {
		...
		if (kvp) {
			client_ame_traverse(ctx, ctx->list, ds->kvp_stack);
		}
...
static int client_ame_accept_key_val(struct ame_decoder *dec,
