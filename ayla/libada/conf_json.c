...
static int conf_json_put_config(jsmn_parser *parser, jsmntok_t *obj, void *arg)
{
    ...
    //giant temporary disable setup_mode from ADS	
	if (!strcmp(name, "sys/setup_mode")) 
	{		
		server_log(LOG_INFO "giant, receive sys/setup_mode value %s, but ignore", val);			
		return  0;	
	}
...
static void conf_json_put_close(struct server_req *req)
{
	...
	server_log(LOG_DEBUG "conf_json_put_close commit");
	client_unlock();
	ada_conf_commit(ctx);
	client_lock();
}
