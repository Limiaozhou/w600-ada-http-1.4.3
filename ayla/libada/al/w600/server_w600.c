    ...
    char *buf, size_t len)
{
	...
	size_t vlen;
	ssize_t rc;
	struct http_state_s *conn;
	char *result;

	AYLA_ASSERT(req->req_impl);
	conn = (struct http_state_s *)req->req_impl;
...
static void server_flush(struct server_req *req, const char *msg)
{
	...
	AYLA_ASSERT(req->req_impl);
	conn = (struct http_state_s *)req->req_impl;
	server_log(LOG_DEBUG "server_flush enter req->len %d, req->err %d\n", req->len, req->err);
...
out:
	server_log(LOG_DEBUG "server_flush out req->len %d, req->err %d\n", req->len, req->err);
	req->len = 0;
}
...
{
	struct http_state_s *hs;
	int rc = 0;

	AYLA_ASSERT(req->req_impl);
	hs = (struct http_state_s *)req->req_impl;
	
	return httpd_request_get_resource(hs, buf, buf_size);
}
	...
	char *buf, size_t size)
{
	...
	AYLA_ASSERT(req->req_impl);
	conn = (struct http_state_s *)req->req_impl;
//printf("server_req_header header %s\n", header);

	rc = httpd_request_get_header_field(conn, header, &value);
	if (rc == 0) {
		//printf("server_req_header value %s\n", value);
	...
	//printf("server_req_header rc %d\n", rc);
	return rc;
}

void server_handler(struct http_state_s *conn)
{
    ...
    if (method == REQ_BAD) {
		server_log(LOG_WARN "Unsupport method failed");
		httpd_response_bad_request(conn, "Unsupport method!");
		goto req_free;
	}
    server_log(LOG_DEBUG "server request: %s", server_method_names[method]);
	...
	if (rc) {
		...
		if (req->user_in_prog) {
			...
			resume_func = (void(*)(struct server_req *)) req->sec_arg;
			resume_func(req);
			if (req->finish_write) {
...
int server_host_match(const struct server_req *req, const char *host)
{
	...
	tls_param_get(TLS_PARAM_ID_WPROTOCOL, (void* )&wmode, FALSE);
	...
	return ipaddr_addr(host) == ethif->ip_addr.addr;
}
...
void server_banner(struct server_req *req, const char *title)
{
