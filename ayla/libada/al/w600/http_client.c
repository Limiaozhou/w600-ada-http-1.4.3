...
#endif
#if       1
static int tls_wait_cnt = 0;
static int tls_wait_flag = 0;
extern void ext_get_cmd(void);
#endif
/*
...
//		now =clock_get(NULL);
//		diff = date - now;
		//printf("hc->http_time = %d - %d = %d \n",date,now,diff);
//		if(diff > 10 || diff < -10){
			//printf("set clock\n");
//			clock_set(hc->http_time,CS_HTTP);
//		}	
	}
}
...
#ifdef JV_CTRL	
		tls_wait_cnt++;
		if(tls_wait_cnt<HTTP_CLIENT_KEEP_OPEN_CNT)
		{
			HTTP_CLIENT_DEBUG(hc, LOG_INFO, "client idle,send something(%d*50s)",tls_wait_cnt);
			ext_get_cmd();//to do something keep tls 
			tls_wait_flag = 1;
		}
		else
		{
			HTTP_CLIENT_DEBUG(hc, LOG_INFO, "client doesn't need conn(%d*50s)",tls_wait_cnt);
			http_client_close(hc);
			tls_wait_cnt = 0;
		}
#else
...
#if  1
		HTTP_CLIENT_LOGF(hc, LOG_WARN,"==>rec");
		if(tls_wait_flag==0)
		{
			tls_wait_cnt = 0;
			HTTP_CLIENT_LOGF(hc, LOG_WARN,"==>clear cnt");
		}
		tls_wait_flag = 0;
#endif
		switch (status) {
...
void http_client_send_pad(struct http_client *hc)
{
	...
	if (hc->sending_chunked || hc->sent_len < 0 || hc->http_tx_chunked) {
		return;
	}
