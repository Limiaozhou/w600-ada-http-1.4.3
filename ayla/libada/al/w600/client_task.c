...
static void client_idle(void *arg)
{
	...
	log_info("ayla agent start");//can not del !
...
int ada_init(void)
{
	...
	log_init();
	//log_mask_init_min(BIT(LOG_SEV_INFO)|BIT(LOG_SEV_DEBUG), LOG_DEFAULT);
	...
	if (xTaskCreateExt(client_idle, "A_Client", (portSTACK_TYPE *)ClientTaskStk, CLIENT_TASK_STACKSZ,
	    NULL, CLIENT_TASK_PRIO, NULL) != pdPASS) {
		AYLA_ASSERT_NOTREACHED();
        printcli("A_Client fail\n");
	}
	client_mdns_init();
	return 0;
}
