...
extern char new_filter_rule;	/*filter prop with input=ture*/
...
client_accept_prop(struct xml_state *sp, int argc, char **argv)
{
	...
	enum ada_err rc;
#if 0	
	if((state->get_all == 1) && (prop ->type != ATLV_SCHED))
		goto clear_prop;
#else
	if((new_filter_rule == 1) && (prop ->type != ATLV_SCHED))
		goto clear_prop;
#endif
...
{
	ASSERT(state->prop_send_cb);
	return state->prop_send_cb(PROP_CB_CONTINUE, 0 ,arg);
}
#endif
...
{
	struct prop_recvd *prop = &prop_recvd;
	enum ada_err (*prop_send_cb)(enum prop_cb_status,u16, void *);
