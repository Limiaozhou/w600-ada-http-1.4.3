				...
				char *buf, size_t len)
{
	...
	size_t vlen;
	ssize_t rc;
	//printf("server_get_arg_by_name req->get_arg_by_name:%x, req->resource %s\n", req->get_arg_by_name, req->resource);
 ...
 */
void server_req_func(struct server_req *req)
{
	...
	buf = tls_mem_alloc(SERVER_BUFLEN);
	if(NULL == buf)
	{
	    server_log(LOG_DEBUG "malloc fail, %s", __func__);
		goto error;
	}
...
//#ifdef ENABLE_HTTP_HEADER_PARSE
	...
	if (req->state == REQ_INIT) {
		...
		req->content_len = 0;
		if (!server_req_header(req, "Content-Length", buf, SERVER_BUFLEN)) {
		...
		if (!server_req_header(req, "User-Agent", buf, SERVER_BUFLEN)) {
			if (server_redir_handler_get) {
				redir = server_redir_handler_get(req,
				    buf);
			}
		}
		if (!server_req_header(req, "Host", buf, SERVER_BUFLEN)) {
			req->host_present = 1;
	...
	if (req->state == REQ_DATA) {
		...
		if (req->url->req_flags & REQ_NO_BUF) {
			 ...
			 */
			server_read(req);
            server_log(LOG_INFO "url_op_1 %x", req->url->url_op);
			req->url->url_op(req);
	...
	if (req->state == REQ_READY) {
		...
		if (req->host_present == 1 && req->host_match == 0) {
            ...
            server_log(LOG_INFO "url_op_2 %x", req->url->url_op);
			...
			const char *url, enum server_method method, u8 priv)
{
	...
	server_log(LOG_DEBUG "server_find_handler method: %d, url: %s", method, url);
	for (i = 0; i < SERVER_URL_GROUPS; i++) {
		...
		for (; tt->url; tt++) {
			//server_log(LOG_DEBUG "url group method: %d, url: %s",tt->method, tt->url);
			if (method == tt->method &&
			    tt->url &&
			    strlen(tt->url) == len &&
	...
	server_log(LOG_DEBUG "url not found!");
