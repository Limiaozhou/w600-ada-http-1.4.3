...
#define stream_socket(sp)	((sp)->enable ? (sp)->sock : (sp)->sock)

#define STREAM_STACK_DEBUG	1

#define STREAM_SSL_INBUF_LEN	1500
#define STREAM_SSL_OUTBUF_LEN	1500
#ifdef  AYLA_CU
#define STREAM_RX_SZ		2000        
#else
...
#define STREAM_TASK_STACKSZ_TLS	((5 * 1024) / sizeof(u32))
#define STREAM_TASK_STACKSZ	((5 * 1024) / sizeof(u32))

//#define STREAM_TASK_PRIO	(tskIDLE_PRIORITY+1+50+1)
...
#define	DEMO_MSG_IDLE_RECEIVE_DATA			4
#define IDLE_QUEUE_SIZE	8
#define TASK_STAY_ONE     0
...
static psX509Cert_t * ca_certs_chain;
static int ca_certs_inited;
#if TASK_STAY_ONE
static OS_STK StreamIdleTaskStk[STREAM_TASK_STACKSZ_TLS];
#endif
static tls_os_queue_t *stream_idle_q;
...
static const char *stream_type_name(struct stream_pcb *ps)
{
	switch (ps->type) {
	case AL_NET_STREAM_TCP: return "TCP";
	case AL_NET_STREAM_TLS: return "TLS";
	case AL_NET_STREAM_UDP: return "UDP";
	case AL_NET_STREAM_DTLS: return "DTLS";
	default:
		break;
	}
	return "XXX";
...
static enum ada_err stream_tx(struct stream_pcb *sp)
{
	...
	if (!sp->enable) {
		...
		len = HTTPWrapperSSLSend(sp->ssl,stream_socket(sp), p, len, 0);
	}
	stream_stack_check(sp);
	if (len < 0) {
...
static void stream_close_sync(struct stream_pcb *sp)
{
	...
	if (sp->enable) {
		if (sp->sock != STREAM_SOCK_INVAL) {
			HTTPWrapperSSLClose(sp->ssl, stream_socket(sp));
			close(stream_socket(sp));
...
static void stream_recv_sync(struct stream_pcb *sp)
{
    ...
    do
    {
    	...
    	if (sp->enable) {
    		...
    		ada_unlock(stream_lock);
    		len = HTTPWrapperSSLRecv(sp->ssl, stream_socket(sp), ada_mbuf_payload(m),
    		    STREAM_RX_SZ, 0);
            full = len;
    		stream_stack_check(sp);
    		ada_lock(stream_lock);
            ...
            if (len == -2)
            {
                ...
                stream_log(sp, LOG_DEBUG2
        			    "line: %d TLS:%d R %d byte(s) at %p\n",__LINE__,
        			    sp->sock, STREAM_RX_SZ, m);
            }
            else
            {
        		if (len <= 0) {
        			if (len < 0) {
        				stream_log(sp, LOG_WARN "line: %d TLS:%d err=-0x%x\n",__LINE__,
        				    sp->sock, -len);
        			}
    		...
    		if (len <= 0) {
    			if (len < 0) {
    				stream_log(sp, LOG_WARN "line:%d TCP:%d err=%d", __LINE__,sock,
    				    len);
    			}
    			goto err;
    		} else {
    			stream_log(sp, LOG_DEBUG2
    			    "line: %d TCP:%d Received %d byte(s) at mbuf %p",__LINE__,
    			    sock, len, m);
    		}
    	}
    	sp->rx_bytes += len;
...
{
	enum ada_err err = 0;
	//struct stream_pcb *sp = NULL;
	int sock = STREAM_SOCK_INVAL;
//	void *msg;
	struct tls_stream_msg *msg;

    while(1)
    {
    	tls_os_queue_receive(stream_idle_q, (void **) &msg, 0, 0);
		switch ((u32) msg->msg)
        {
            case DEMO_MSG_IDLE_RECEIVE_DATA:
			{
				...
				{			
					...
					ada_lock(stream_lock);
					sp = msg->data;
					stream_log(sp, LOG_DEBUG2 "idle %s PCB=%p", stream_type(sp), sp);
					...
					stream_event_pend(sp);
					ada_unlock(stream_lock);
					//tls_os_time_delay(HZ*5); //for sake of a next connecting.
				}
				tls_mem_free(msg);
        	}
            break;
...
{
	int err;
	int flags = CERT_STORE_UNPARSED_BUFFER;

	if ((err = psX509ParseCert(NULL, certs, (uint32)size, &ca_certs_chain,
				flags)) < 0) {
			psX509FreeCert(ca_certs_chain);
			ca_certs_chain = NULL;
			return err;
		}
	return 0;
...
void tls_stream_send_msg(u32 msg, void *data)
{
    struct tls_stream_msg *pmsg;

    pmsg = tls_mem_alloc(sizeof(struct tls_stream_msg));
    if (NULL != pmsg)
    {
        memset(pmsg, 0, sizeof(struct tls_stream_msg));
        pmsg->msg = msg;
        pmsg->data = data;
        tls_os_queue_send(stream_idle_q, pmsg, 0);
    }
	...
	struct tcp_metric_conn *tcp_metric)
{
    ...
    {
        tmp = 0;
		tls_os_queue_create(&(stream_idle_q), IDLE_QUEUE_SIZE);
    	if (xTaskCreateExt(stream_idle,
    	    "A_Stream",
    	    (portSTACK_TYPE *)StreamIdleTaskStk,
    	    //sp->enable ? STREAM_TASK_STACKSZ_TLS : STREAM_TASK_STACKSZ,
    	    STREAM_TASK_STACKSZ_TLS,
    	    NULL,
    	    STREAM_TASK_PRIO,
    	    &sp->task) != pdPASS) 
    	{
...
//	tls_os_queue_send(stream_idle_q, (void *) DEMO_MSG_IDLE_RECEIVE_DATA, 0);
	tls_stream_send_msg(DEMO_MSG_IDLE_RECEIVE_DATA, sp);
#else
    ...
    u8_t flags)
{
	...
	if (sp->tx_buf) {
		stream_log(sp, LOG_INFO "%s: tx paused", __func__);
		return AE_BUF;
	}
	m = ada_mbuf_alloc(len);
	if (!m) {
		stream_log(sp, LOG_INFO "%s: can't alloc buf", __func__);
		return AE_BUF;
	}
	...
	ada_unlock(stream_lock);
	if (err) {
		stream_log(sp, LOG_INFO "%s: ret err %d", __func__, err);
	}
	stream_log(sp, LOG_DEBUG2 "%s:%d sent %d byte(s)", stream_type(sp),
...
static int stream_ssl_verify(struct stream_pcb *sp,
	psX509Cert_t *crt, int depth, uint32_t *flags)
{
	return 0;
}
...
static enum ada_err stream_tcp_connected(struct stream_pcb *sp)
{
	...
	conn_err = sp->rx_err;
	stream_log(sp, LOG_DEBUG2 "%s:%d Connected err=%s", stream_type(sp),
...
static enum ada_err stream_connect_sync(struct stream_pcb *sp)
{
	...
	int rc;
	ULONG           Address = 0;
    HTTP_SOCKADDR_IN ServerAddress;
	struct sockaddr_in s_in;

    stream_log(sp, LOG_DEBUG2
			    "sp->enable %d", sp->enable);
	if (sp->enable) {
		/* HTTPS session */
		//mbedtls_net_init(&sp->ssl_fd);
		sp->sock = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
		if(sp->sock < 0)
		{
			stream_log(sp, LOG_ERR
		...
		rc = HostByName(sp->host, &Address);
		if (!rc) {
			stream_log(sp, LOG_ERR
			    "HostByName fail, err=%d", rc);
			sp->rx_err = AE_TIMEOUT;
		}
		memset(&ServerAddress, 0, sizeof(HTTP_SOCKADDR_IN)); 
		ServerAddress.sin_family        = AF_INET;
		ServerAddress.sin_addr.s_addr   = Address; 
		ServerAddress.sin_port      = htons(sp->remote_port);
		rc = HTTPWrapperSSLConnect(&sp->ssl,stream_socket(sp),	// Socket
		...
		if (rc) {
			stream_log(sp, LOG_ERR
			    "HTTPWrapperSSLConnect fail, err=%d", rc);
			sp->rx_err = AE_TIMEOUT;
		}
	} else {
		/* HTTP session */
		...
		for (tdelay = 0; tdelay < STREAM_SOCK_RETRY_TIME;
			...
			//vTaskDelay(delay);
			printf("zzz\r\n");
            tls_os_time_delay(delay*HZ/1000);
		}
		ada_lock(stream_lock);
		...
		ada_unlock(stream_lock);
        stream_log(sp, LOG_DEBUG2 "TCP:%d socket is connecting",sp->sock);
		rc = connect(sp->sock, (struct sockaddr *)&s_in, sizeof(s_in));
        stream_log(sp, LOG_DEBUG2 "TCP:%d socket connect return",sp->sock);
		ada_lock(stream_lock);
		stream_stack_check(sp);
		if (sp->tcp_metric) {
