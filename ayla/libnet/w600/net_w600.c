...
static OS_STK NetPollTaskStk[NET_UDP_TASK_STACKSZ];
static OS_STK NetTimerTaskStk[NET_TIMER_TASK_STACKSZ];
...
void net_init(void)
{
	...
	if (xTaskCreateExt(net_poll_wait, "A_NetPoll", (portSTACK_TYPE *)NetPollTaskStk, NET_UDP_TASK_STACKSZ, 
	...
	if (xTaskCreateExt(net_timer_wait, "A_NetTime", (portSTACK_TYPE *)NetTimerTaskStk,
    ...
    struct net_callback *cb)
{
...
#else
	u8 error = xQueueSendFromISR(cbq, &cb, TX_NO_WAIT);
    if( error!=1 ) {
	...
	u8 isrcount = tls_get_isr_count();
	if(isrcount > 0)
	{
		error = xQueueSendFromISR(cbq, &cb, &pxHigherPriorityTaskWoken );
	...
	{
		error = xQueueSend(cbq, &cb, TX_NO_WAIT );
	}
    if( error!=1 ) {
        printf("xQueueSendFromISR error ....\r\n");
    }
