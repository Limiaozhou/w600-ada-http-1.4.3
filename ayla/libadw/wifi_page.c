...
printf("adw_wifi_add_prof sec_token %d, key_len %d, None: %d, wep: %d\n", sec_token, key_len,CT_none, CT_WEP );
	...
	adw_log(LOG_DEBUG "add_prof ssid %s sec %u %s%s, key:%s",
	    adw_format_ssid(ssid, ssid_buf, sizeof(ssid_buf)),
	    sec_token, mfi ? "mfi " : "", hidden ? "hidden " : "", prof->key);
...
static void adw_wifi_json_connect_post(struct server_req *req)
{
	...
	u8 hidden = 0;
printf("adw_wifi_json_connect_post enter\n");
...
printf("scan->wmi_sec %d, ssid %s\n", scan->wmi_sec, scan->ssid.id);
