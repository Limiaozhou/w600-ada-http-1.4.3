...
#define USER_ADDR_START		(0xF0000)

#define START_CONF_SIZE		(16 * 1024)
#define FACTORY_CONF_SIZE	(16 * 1024)

#define FACTORY_CONF_ADDRESS (FLASH_BASE_ADDR + USER_ADDR_START + FACTORY_CONF_SIZE)
#define START_CONF_ADDRESS	 (FLASH_BASE_ADDR + USER_ADDR_START)
...
//added 20180605
struct flash_dev *conf_flash_open(enum conf_dev dev_type)
{
    struct flash_dev *flash_obj;
    return flash_obj;   
}

static int wm_flash_read(uint dset_handle, u8 *buffer,
    u32 len, u32 offset)
{
	flash_t flash;

	if (len > 0) {
		//printcli("read: %x %x %x\n", dset_handle, offset, len);
        tls_fls_read(dset_handle+offset, buffer, len);
	}

	return 0;
}

static int wm_flash_write(uint dset_handle, u8 *buffer,
    u32 len, u32 offset)
{
	flash_t obj;

    //printcli("write: %x\n", offset);
    tls_fls_write(offset, buffer, len);
	...
	void *buf, size_t len)
{
	...
	status = wm_flash_read(file->loc, buf, len, offset);
	if (status != 0) {
		return NULL;
	}
	return buf;
	...
	void *buf, size_t len)
{
	...
	status = wm_flash_write(file->loc, buf, len, offset);
	if (status != 0) {
		return -1;
	}
	state->written = 1;
...
int conf_flash_erase(enum conf_inode inode)
{
	...
	case CI_STARTUP:
        ...
        sector = (address/INSIDE_FLS_SECTOR_SIZE);

		tls_fls_erase(sector+0);
        tls_fls_erase(sector+1);
        tls_fls_erase(sector+2);
        tls_fls_erase(sector+3);
		break;
	default:
		break;
	}
